...
import { Plus, Edit, Trash2, Contact } from 'lucide-react';
...
export default function DesignationsPage() {
  ...
  const canManage = can('hrm.designations.manage');
  ...
  const load = async () => {
    try {
      const [d, dep] = await Promise.all([
        apiJson('/api/admin/hrm/designations'),
        apiJson('/api/admin/hrm/departments'),
      ]);
  ...
  const openEdit = (d) => { setEditing(d); setForm({ name: d.name, department_id: d.department_id ?? '', description: d.description || '', is_active: !!d.is_active }); setShowForm(true); };

  const submit = async (e) => {
    e.preventDefault();
    if (!form.name.trim()) { addToast(friendlyMessage('validation', { description: 'Give the designation a name.' })); return; }
    setSaving(true);
    try {
      await apiJson('/api/admin/hrm/designations', { method: editing ? 'PUT' : 'POST', body: JSON.stringify({ ...form, id: editing?.id }) });
      addToast(friendlyMessage('save_success', { description: editing ? 'Designation updated.' : 'Designation added.' }));
  ...
  const remove = async (d) => {
    const ok = await confirm({ title: `Delete "${d.name}"?`, message: d.staff_count > 0 ? `${d.staff_count} staff member(s) will be unassigned from this designation.` : undefined, tone: 'delete' });
    if (!ok) return;
    try { await apiJson(`/api/admin/hrm/designations?id=${d.id}`, { method: 'DELETE' }); load(); }
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mx-auto w-full max-w-4xl">
          <div className="mb-6 flex flex-col gap-3 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-800 sm:text-3xl">Designations</h1>
              <p className="mt-1 text-gray-700">Job titles staff can be assigned — Waiter, Head Chef, Shift Lead.</p>
            </div>
            {canManage && (
              <button onClick={openCreate} className="flex items-center gap-2 rounded-lg bg-gray-900 px-6 py-3 text-white hover:bg-gray-800">
                <Plus className="h-5 w-5" /> <span>Add Designation</span>
              </button>
            )}
          </div>
          ...
          {showForm && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
              <div className="w-full max-w-lg rounded-2xl bg-white p-6 sm:p-8">
                <h2 className="mb-5 text-xl font-bold text-gray-800">{editing ? 'Edit Designation' : 'Add Designation'}</h2>
                <form onSubmit={submit} noValidate className="space-y-4">
                  <label className="block">
                    <span className="mb-1 block text-sm font-medium text-gray-700">Designation name *</span>
                    <input autoFocus value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="h-11 w-full rounded-lg border border-gray-300 px-3 text-gray-900" placeholder="e.g. Head Waiter" />
                  </label>
                  <label className="block">
                    <span className="mb-1 block text-sm font-medium text-gray-700">Department</span>
                    <select value={form.department_id} onChange={(e) => setForm({ ...form, department_id: e.target.value })} className="h-11 w-full rounded-lg border border-gray-300 px-3 text-gray-900">
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
              ...
              <tbody className="divide-y divide-gray-200">
                {designations.map((d) => (
                  <tr key={d.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-gray-700">{d.department_name || '—'}</td>
            ...
            {!loading && designations.length === 0 && (
              <div className="py-12 text-center"><Contact className="mx-auto mb-4 h-12 w-12 text-gray-400" /><p className="text-gray-700">No designations yet. Add one like Waiter or Head Chef.</p></div>
            )}
          </div>
        </div>
      </div>
