...
export async function postExpenseJournal(db, expense) {
  ...
  const { prepareExpenseFunding } = await import('@/lib/business-funding.js');
  const funding = await prepareExpenseFunding(db, expense, creditCode);

  if (creditCode === '1050') {
    const date = expense.expense_date || expense.date;
    const available = round2(await accountBalanceAsOf(db, '1050', date));
    if (available + 0.001 < amount) {
      throw Object.assign(
        new Error(`Business Funding has Rs ${available.toFixed(2)} available on ${date}, but Rs ${amount.toFixed(2)} is required.`),
  ...
  const journalId = await postJournal(db, {
    ...
    lines: [
      ...
      { code: creditCode, debit: 0, credit: amount, drawer_id: drawerId, bank_account_id: bankAccountId, supplier_id: supplierTag },
    ],
  });
  return { journalId, businessFundingAmount: funding.amount || 0 };
}
