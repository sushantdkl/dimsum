...
export async function correctExpensePaymentMethod(db, {
  ...
  return db.transaction(async (tx) => {
    ...
    if (nextCode === '1050') {
      const available = round2(await accountBalanceAsOf(tx, '1050', entryDate)) + originalFunding;
      if (available + 0.001 < amount) {
        throw Object.assign(
          new Error(`Business Funding has Rs ${available.toFixed(2)} available on ${entryDate}, but Rs ${amount.toFixed(2)} is required.`),
