...
async function spending(db, from, to, condition) {
  ...
  for (const r of rows) {
    ...
    else if (['credit', 'due', 'unpaid', 'on_credit', 'payable'].includes(method)) out.credit += num(r, 'amount');
    else if (['owner_pocket', 'business_funding'].includes(method)) out.funding += num(r, 'amount');
    else out.bank += num(r, 'amount');
    out.records += num(r, 'records');
  }
  out.cash=round2(out.cash); out.bank=round2(out.bank); out.credit=round2(out.credit); out.funding=round2(out.funding); out.total=round2(out.cash+out.bank+out.credit+out.funding);
  return out;
}
 ...
 * Opening adjustments are already represented by the declared physical
