...
export async function businessDaySummary(db, dayOrId) {
  ...
  const cashRange = session
    ...
    : { sql: '', params: [] };
  // Revenue retains the originating order's business day. Cash is reconciled
  // in the drawer session in which it was physically received, so an active
  // session uses its journal/time range even for a carried order from yesterday.
  ...
  const [sales, channels, paymentRows, corrections, expenses, orders, kots, itemTotals, topItems, tables, cashRows, allDayCashRows, postCloseCashSalesRow, onlineRows, sessionStats, blockers] = await Promise.all([
